...
*/
		...
		void OnSceneGUI () {
			if(light){
				...
				for (int i = 0; i <= 2 ; i++)
				{
					...
					#if UNITY_5_6_OR_NEWER
						var fmh_657_7_638786721676253800 = Quaternion.identity; newHandlePoint = (lTransform.position - Handles.FreeMoveHandle(
				...
				var fmh_750_79_638786721676272926 = Quaternion.identity; Vector3 newFreeHandlePoint = (Handles.FreeMoveHandle(lTransform.position,
		...
		private void generateAddOnsButtons(){
            ...
            searchString = EditorGUILayout.TextField(searchString, EditorStyles.toolbarSearchField, GUILayout.Width(150));
		...
		*/
		...
		void PerformAndDrawAngleHandles(DynamicLight2D.DynamicLight light)
		{
			...
			#if UNITY_5_6_OR_NEWER
			var fmh_1104_53_638786721676284770 = Quaternion.identity; Vector3 cwHandle = Handles.FreeMoveHandle(cwPos, HandleUtility.GetHandleSize(light.transform.position) * .04f, Vector3.zero, Handles.CircleHandleCap);
            #else
			...
			#if UNITY_5_6_OR_NEWER
			var fmh_1121_45_638786721676289501 = Quaternion.identity; cwHandle = Handles.FreeMoveHandle(cwPos, HandleUtility.GetHandleSize(light.transform.position) * .04f, Vector3.zero, Handles.CircleHandleCap);
			#else
